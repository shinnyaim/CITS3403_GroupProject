...
class Cloud {
  ...
  /*Every frame moves the cloud right by its speed. Whne it goes past 200px past the right edge (offcscreen),
  it teleports to 200px past the left edge and gets a new random y. The 200px buffer means clouds don't pop in 
  and out abruptly at the edges, but instead fade in and out smoothly. */
  update() {
    this.x += this.speed;
  ...
  draw() {
    ...
    ctx.drawImage(cloudImage, this.x, this.y, 200 * this.scale, 120 * this.scale);
    ctx.globalAlpha = 1;
  }
}
